...
void jl_crypt(uint8_t *ptr, int len, uint16_t key) {
	while (len) {
		*ptr++ ^= key; len--;
		...
		// this is actually `key = (key << 1) ^ (key >> 15 ? 0x1021 : 0);`
		// but i'll still leave this for historical preservance.
		//                                                  [[06.02.2023]]
		key = (((key>>15)<<4^(key&255))<<1|(key>>15 )) |
		      (((key>>15)<<3^(key>>8 ))<<1|(key>>7&1)) << 8;
	}
}
...
uint16_t jl_crc16(uint8_t *data, int len, uint16_t sr) {
	while (len--) sr = (sr << 8) ^ crc16tab[(sr >> 8) ^ *data++];
	return sr;
}
